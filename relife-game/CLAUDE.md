...
- `useGameSync.ts`: 處理雙向同步，避免循環更新（同步 pendingFunctionCard、pendingDiscard）
...
| `aiShouldUseInvalidCard(card, player)` | 威脅判斷：高威脅必擋、中威脅 70%、低威脅 30% |
...
- **資料定義**（`data/achievements.ts`）：成就門檻、唯一成就定義 + `evaluateAchievements()` 純函數
...
- **隨機事件**: 其他回合從事件池抽取
- 大部分事件會影響全員的金錢或能力值
- **事件彈窗會顯示目標玩家**：「最有錢/最窮」事件會列出具體玩家名字與金額
...
### 已實作
- 15 個角色（完整版）
- 9 種職業（完整版：智力型 3 種、體力型 3 種、魅力型 3 種）
- 30 回合遊戲（根據桌遊規則）
- 175 張卡牌（根據桌遊規則）
- 3 個探險地點
- 事件系統（事件彈窗顯示目標玩家名稱與金額）
...
- 成就系統（門檻成就 + 唯一成就，結算畫面顯示成就標籤與加成分數）
...
Engine Layer 的純函數測試，共 170 個測試案例：

| 檔案 | 測試數 | 涵蓋功能 |
|------|--------|----------|
| `calculator.test.ts` | 35 | 分數計算、金錢/屬性變化、玩家查詢、勝利條件、成就判定 |
| `jobSystem.test.ts` | 27 | 應徵、離職、升遷、薪水、職業技能 |
| `cardEffects.test.ts` | 25 | 卡牌使用檢查、效果套用、特殊效果（park_bad/good、應酬、偷睡覺） |
| `gameLogic.test.ts` | 45 | 回合流程、抽牌、棄牌、溢出偵測、事件效果（7 種特殊事件 + 一般效果） |
